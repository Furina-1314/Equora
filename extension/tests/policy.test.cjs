...
test('Host state blocks current tabs, temporary exception works, disconnect clears rules', async () => {
  ...
  const status = () => {
    let value;
    messages.fn({ type: 'getStatus' }, { url: 'chrome-extension://test/popup.html' }, response => { value = response; });
    return value;
  };
  assert.equal(status().connected, false);
  let untrustedResponse = false;
  messages.fn({ type: 'getStatus' }, { url: 'https://example.com/popup.html' }, () => { untrustedResponse = true; });
  assert.equal(untrustedResponse, false);
  port.onMessage.fn({ type: 'state', state: { enabled: true, blockedDomains: ['example.com'], updatedUtcMs: Date.now() } });
  await settle();
  assert.equal(status().connected, true);
  assert.equal(status().enabled, true);
  assert.deepEqual(Array.from(status().blockedDomains), ['example.com']);
  assert.equal(rules[0].action.type, 'redirect');
  for (const original of ['https://example.com/a/b?x=1&y=2#part', 'https://example.com:8443/path']) {
    const redirect = original.replace(new RegExp(rules[0].condition.regexFilter), rules[0].action.redirect.regexSubstitution.replace('\\1', '$1'));
    assert.equal(new URL(redirect).searchParams.get('host'), 'example.com');
  }
  ...
  port.onMessage.fn({ type: 'state', state: { enabled: true, blockedDomains: ['example.com'], updatedUtcMs: Date.now() - 13000 } });
  assert.equal(status().fresh, false);
  assert.equal(status().blockedDomains.length, 0);
