...
## P15 — Windows 深度集成与 M8 加固(M9+M8 剩余)(2026-09-19)

### 已完成

- **SafetyWhitelist(需求 §12.3)**:系统设置/安全软件/文件管理器/辅助功能/
  紧急通信/恢复工具/Equora 自身共 20+ 关键进程永不被限制(可维护扩充);
  SanitizeBlockedList 在规则加载前剥离受保护进程(防御性设计,黑名单污染不会
  锁死系统)。测试 12 例(逐进程验证 + 过滤 + 边界)。
- **EmergencyUnlock**:紧急解锁安全出口(触发后冷却期 15 分钟内解除全部应用
  限制;幂等不重置冷却;审计计数;过期后可再触发)。测试 4 例。
- **StartupRegistration**:HKCU Run 键注册(不写 HKLM 避免权限;
  SetRegistered(true/false) + IsRegistered 往返验证)。
- **服务端 RateLimiter(令牌桶)**:每秒 N 次 + 桶容量 B;突发后拒绝(429
  语义);按时间恢复;不超过桶容量。测试 3 例。
- **服务端 AuditLog**:认证失败/限流/同步/设备撤销事件,脱敏(令牌 *** +
  设备 UUID[:8]),不含任务正文(需求 §24)。测试 2 例(含脱敏验证)。

### 构建与测试结果(本机实测)

- 服务端 20/20(新增 5);C# 105/105(新增 19);原生 127/127 不变。
- 修复:EmergencyUnlock 解锁期内再触发现在也计数(审计语义)。

### 已知问题 / 假设

- Windows Service + 命名管道严格限制、Widgets、通知操作按钮、MSIX 打包、
  HTTPS 终端、令牌刷新为平台绑定工作,需真机环境验证,归入 P16 发布阶段
  统一落地(基础安全设计已就绪:白名单/紧急解锁/审计)。

### 下一步

- P16(M10):性能优化、无障碍与多 DPI、MSIX 打包、备份恢复演练、
  故障注入、用户指南与隐私说明、签名发布。

---

## P16 — 发布候选(M10)(2026-09-19)—— 全部里程碑达成

### 已完成

- **性能基准(10 例,Release 构建)**:迁移、任务创建 1000 条 P95、
  智能清单查询、全量列表、一致性备份、周窗口物化 500 块。
- **故障注入(5 例)**:数据库锁、备份截断、不可写路径、迁移断电、空文件。
- **用户指南**(docs/user-guide.md):快速开始、全部功能、隐私、FAQ、快捷键。
- **隐私与安全说明**(docs/privacy-and-security.md)。
- **第三方组件声明**(THIRD-PARTY-NOTICES.md)。
- **发布说明**(docs/release-notes.md):功能矩阵、262 例测试、性能实测、已知限制。

### 项目总结(P0–P16 全部完成,M0–M10 全部达成)

- 仓库:65+ 提交,3 条 CI 全绿,262 例测试全部通过。
- 数据库:6 个版本化迁移,故障不丢数据,备份带校验。
- 架构:Domain←Storage←Core←CApi 分层,零警告编译。

---

## P17 — v0.2.0 使用限制与桌面体验升级(2026-09-20)

### 已完成

- **应用与网站限制**:RestrictionsPage + RestrictionPolicy/RestrictionRuntime;
  前台窗口每秒检查(命中最小化不杀进程、空闲 60 秒暂停统计、安全白名单);
  浏览器侧 MV3 扩展策略引擎(policy.js)+ 本地消息组件(ForegroundAccess,
  仅当前用户注册),域名含子域名、活动标签计时、临时允许 5 分钟、心跳 12 秒超时解除。
  条件:专注期间禁止 / 每日时间段(可跨午夜)/ 每日分钟额度,任一满足即限制;
  "暂停所有限制 15 分钟"。
- **任务分类**:项目/标签管理(名称/颜色),任务分配,删除分类保留任务。
- **专注暂停/继续**:冻结计时跨页面保持,恢复续跑;新增工作/休息轮次等模式。
- **界面刷新**:紧凑内联 SplitView 导航、主题/强调色(preferences.json,
  解包构建同样生效)、高对比度跟随系统;任务草稿字段、选中/详情状态一致性、
  搜索防抖、Ctrl+Z 撤销;日历时间块编辑(备注/颜色/时间,删块保留任务)。
- **托盘与单实例**:关闭到托盘、再启动唤起已有实例、可恢复常规退出。
- **数据目录迁移**:DataDirectoryMigration 快照迁移 + 非覆盖保护。
- **打包(v0.2.0 双格式)**:desktop/packaging/build-release.ps1 ——
  dotnet publish 自包含(WindowsPackageType=None)→ makeappx 手工组包 +
  signtool 签名(MSIX);Inno Setup EXE 安装器(用户级安装免管理员,同签名);
  BrowserExtension.zip 与 SHA256SUMS.txt。MSIX 布局补 resources.pri
  (解包发布产物名为 Equora.App.pri,两者并存兼容两种运行方式)。

### 构建与测试结果(本机实测)

- C# 127/127(新增 22);扩展策略 2/2 + Host 协议 1/1;原生 137、服务端 20 不变
  (合计 287)。
- Release 产物冒烟:解包 exe 与已安装 MSIX 均正常启动出窗(本机实测安装
  v0.2.0 覆盖 v0.1.1)。
- v0.2.0 以 pre-release 发布(MSIX 手工布局 + EXE 安装器链路首次上线)。

### 已知问题 / 假设

- 应用限制为自主管理(最小化),非系统级防绕过;需保持 Equora 运行。
- 浏览器扩展未上架商店,需开发者模式加载(应用内提供指引)。
- 安装包仍为自签名证书;SmartScreen 可能告警。

### 下一步

- 正式版方向:代码签名证书、HTTPS 终端、Widgets/通知操作、扩展商店发布。
