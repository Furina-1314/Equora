...
填写 `example.com` 同时限制其子域名，不影响 `notexample.com`。按当前获得焦点的浏览器窗口中的活动网页统计，每约 5 秒更新一次，不读取网页正文或保存 URL 路径。后台标签不计时，空闲超过 60 秒暂停计时。达到条件后，新打开与已经打开的匹配网页均转到限制页；限制页可临时允许该网站一次、共 5 分钟，然后返回网站首页；网页右上角显示共享倒计时。倒计时结束后恢复限制并禁用再次临时允许，刷新页面或重启浏览器不会重置使用记录。同一规则覆盖的子域名共享这次机会。
